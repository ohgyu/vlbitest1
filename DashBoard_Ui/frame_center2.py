...
class FrameCenter(QFrame):
    def __init__(self, parent=None, frame_right=None):
        ...
        # frame_right 인자는 더 이상 사용하지 않지만, 기존 호환성을 위해 남겨둠
        self.setStyleSheet("background-color:#0F172A;")

        self.selected_parents = []
        self.current_parent = None

        # 여러 parent에서 child 선택 지원
        self.selected_children = {}  # { parent: [child1, child2, ...] }
        ...
        # 제목 라벨
        self.title_label = QLabel("통계 대시보드")
        self.title_label.setStyleSheet(
            "color:rgb(110,227,247); font-size:20pt; font-weight:bold;"
        )
        ...
        # ★ 잘못된 시그니처 수정: reload_data → refresh_all_data
        ...
        self.toolbar.setStyleSheet(
            """
            QToolBar { background-color: #0F172A; border: none; }
